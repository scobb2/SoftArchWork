...
.L3:
	...
	mov	r3, r3, asl #1		@ BUG asl #2		@ multiplies r3 ('idx') by 4 to get the offset in bytes of unique primeSet array value
...
.L9:
	...
	ldr	r1, [fp, #-8]		@ BUG fp, #-12 		@ loads 'toTest' into r1 to prepare for the print
...
.L13:
	...
	bne	.L16				@ BUG .L15 			@ if ('toTest' % 'divisor') != 0, branches to .L15 (incrementing 'divisor')
.L14:
	ldr	r2, [fp, #-8]							@ loads 'divisor' into r2.... after the for loop in the C code
	ldr	r3, [fp, #-16]							@ loads 'toTest' into r3
	cmp	r2, r3									@ compares 'divisor' and 'toTest'
	beq	.L16				@ BUG  bne			@ if 'divisor' != 'toTest', branches to .L16 (this branches around the if-statement)
